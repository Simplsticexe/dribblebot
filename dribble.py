...
@is_admin()
async def close(ctx):
    await ctx.send("Closing ticket in 5 seconds 🗑️")
...
async def setupmm(ctx):
    ...
    panel_embed = discord.Embed(
        ...
        description=(
            "⬝ Click the blue **Request Middleman** button below to create a ticket.\n\n"
            "**How Middleman Works:**\n"
...
async def completed(ctx, action=None, channel: discord.TextChannel=None):
    if action == "set" and channel:
        ...
        await ctx.send(f"✅ completed logs will now send to {channel.mention}")
    else:
